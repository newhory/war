        ...
        public void OnUpdate(ref SystemState state)
        {
...
#if UNITY_EDITOR
                        UnityEngine.Debug.DrawLine(dragStartPosition, onPointerDragging.PositionOnGround, UnityEngine.Color.red);
#endif
                    }
                }
            }
            ...
            if (state.EntityManager.IsComponentEnabled<OnPointerPressEnd>(PointInput))
            {
                ...
                if (state.EntityManager.IsComponentEnabled<DragStartWorldPosition>(PointInput))
                {
                    ...
                    state.EntityManager.SetComponentData(PointInput, new DragEndWorldPosition { Position = onPointerDragEnd.PositionOnGround });

                    (Entity currentPickedTroop, TeamColor teamColor) = GetPickedTroop(ref state, onPointerDragEnd.PositionOnGround);
                    ...
                    if (CurrentSelectedEntity != currentPickedTroop)
                    {
                        if (currentPickedTroop == Entity.Null) // CurrentSelectedEntity is not Null
                        {
                            ...
                            state.EntityManager.SetComponentData(CurrentSelectedEntity, new TroopFormationReset { TroopPosition = onPointerDragEnd.PositionOnGround });
...
#if UNITY_EDITOR
                    UnityEngine.Debug.DrawLine(onPointerDragEnd.PositionOnGround, onPointerDragEnd.PositionOnGround + math.up() * 2.5f, UnityEngine.Color.blue, 5f);
#endif
